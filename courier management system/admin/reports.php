...
table{
  ...
  <section class="p-5" style="background-color: white; border-top: 4px solid #D54E16; border-radius: 5px; box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px;">
    ...
    $(document).ready(function () {
        ...
        $read = "SELECT * FROM parcel WHERE date(date_created) BETWEEN '$from_date' AND '$to_date'" . ($parcel_status != 'ALL' ? " AND parcel_status = $parcel_status" : "") . " ORDER BY unix_timestamp(date_created) ASC;";
...
<div class='col-md-2 mx-auto my-3'>
        <a href="./reportgeneration/generate_excel.php?parcel_status=<?= $parcel_status ?>&from_date=<?= $from_date ?>&to_date=<?= $to_date ?>" download="report.xlsx" class='btn btn-success' id='print'><i class='fa fa-print'></i> Print</a>
